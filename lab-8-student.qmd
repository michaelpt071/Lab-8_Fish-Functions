...
blackfoot_fish <- read_csv("BlackfootFish.csv")
```
...
blackfoot_fish %>% 
  summarize(across(c(trip, mark, length, weight, year, section, species), ~ sum(is.na(.))))
...
blackfoot_fish %>% 
  group_by(year) %>% 
  summarize(across(c(trip, mark, length, weight, section, species), ~ sum(is.na(.)))) %>% 
  select(year, weight) %>% 
  ggplot(aes(x = year, y = weight)) +
  geom_line() +
  labs( x = "Year",
  y = "Missing Values (Weight)")+
  theme_classic()
```
...
> Some of the variable specifications in the min and max functions are erroneous.
